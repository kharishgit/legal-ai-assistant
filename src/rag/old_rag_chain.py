...
def load_from_cache(query):
    print(f"Loading from cache for query: {query}")
    cache_file = os.path.join(CACHE_DIR, f"{get_cache_key(query)}.pkl")
    if os.path.exists(cache_file):
        with open(cache_file, "rb") as f:
            return pickle.load(f)
...
def save_to_cache(query, result):
    print(f"Saving to cache for query: {query}")
    cache_file = os.path.join(CACHE_DIR, f"{get_cache_key(query)}.pkl")
    with open(cache_file, "wb") as f:
        pickle.dump(result, f)
...
def initialize_rag_chain():
    ...
    try:
        ...
        # Wrapper to log and validate retriever output
        def process_retriever_output(docs):
            logger.info(f"Retriever output: {json.dumps(docs) if isinstance(docs, (dict, list)) else str(docs)}")
            if not docs:
                logger.warning("Retriever returned empty result")
                return ""
            valid_texts = []
            for doc in docs:
                ...
                elif isinstance(doc, dict) and 'page_content' in doc and isinstance(doc['page_content'], str) and doc['page_content'].strip():
                    valid_texts.append(doc['page_content'])
                else:
                    logger.warning(f"Invalid document in retriever output: {str(doc)}")
...
Answer:"""
        ...
        # Create RAG chain with robust chunking
        def chunk_context(context):
            logger.info(f"chunk_context received context: {json.dumps(context) if isinstance(context, (dict, list)) else str(context)}")
            if context is None or (isinstance(context, list) and not context):
                logger.warning("Context is None or empty list, returning empty string")
                return ""
            if isinstance(context, list):
                valid_texts = []
                for doc in context:
                    if hasattr(doc, 'page_content') and isinstance(doc.page_content, str) and doc.page_content.strip():
                        ...
                        logger.warning(f"Invalid document in context: {str(doc)}")
                context = " ".join(valid_texts) if valid_texts else ""
            elif not isinstance(context, str):
                context = str(context)
            logger.info(f"Processed context for splitting: {context[:100]}...")
            chunks = text_splitter.split_text(context) if context else [""]
            logger.info(f"Generated chunks: {chunks}")
            return "\n".join(chunks[:3])

        chain = (
            {"context": lambda x: process_retriever_output(retriever_with_metadata(x["question"], x.get("metadata")).get_relevant_documents()), 
